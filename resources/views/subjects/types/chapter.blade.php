              ...
              <div class="flex items-center py-4 border-t border-gray-100">
                ...
                <div class="w-1/3">
                  <div class="flex items-center justify-between px-5">
                    ...
                    <button type="button" x-data x-on:click="$dispatch('start-exam', {show: true, storeUrl: '{{ route('paperRecords.store', $paper) }}', totalitiesUrl: '{{ route('papers.getTotalities', $paper) }}'})" class="px-3 py-1 text-sm flex items-center justify-center border-2 border-indigo-500 text-indigo-500 bg-indigo-50 rounded focus:outline-none">马上练习</button>
                  </div>
                </div>
              </div>
                  ...
                  <div class="flex items-center py-4 border-t border-gray-100">
                    ...
                    <div class="w-1/3">
                      <div class="flex items-center justify-between px-5">
                        ...
                        <button type="button" x-data x-on:click="$dispatch('start-exam', {show: true, storeUrl: '{{ route('paperRecords.store', $childrenPaper) }}', totalitiesUrl: '{{ route('papers.getTotalities', $childrenPaper) }}'})" class="px-3 py-1 text-sm flex items-center justify-center border-2 border-indigo-500 text-indigo-500 bg-indigo-50 rounded focus:outline-none">马上练习</button>
                      </div>
                    </div>
                  </div>
                @endforeach
